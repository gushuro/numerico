...
function [L2, H1] = ej1(N, graficar, graficarerror)
    %N=10;
    M=N;
    ...
    if graficarerror
        trimesh(elem_vertices, vertex_coordinates(:,1), vertex_coordinates(:,2), abs(uh-UpostaVector))
        %[uh,UpostaVector]
    end
    ...
    for i = 1:cantTriang
        ...
        aprox = (uh(numv1)+ uh(numv2) + uh(numv3))/3;
        errorU = errorU + med*((ufun(baricentro(1),baricentro(2)) - aprox)^2);

        gradi = uh(numv1) * ([-1,-1]*inversasB(:,[2*i-1,2*i])) + uh(numv2) * ([1,0]*inversasB(:,[2*i-1,2*i])) + uh(numv3) * ([0,1]*inversasB(:,[2*i-1,2*i]));
        dif = gradi - gradufun(baricentro(1),baricentro(2));
        errorGrad = errorGrad + (dif(1)^2 + dif(2)^2)*med;

    end
    L2 = sqrt(errorU)
    H1 = sqrt(errorGrad + errorU)
    %for i = 1:cantNodos
    %    e = grad(i,:)-gradufun(vertex_coordinates(i,1), vertex_coordinates(i,2));
    %    errorH1 = errorH1 + (e(1)^2 + e(2)^2);   

    %errorGrad = sqrt(errorGrad)
end
